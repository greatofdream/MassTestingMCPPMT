...
, in which $\mu_{\mathrm{ser}}$ is the time offset of the pulse, $\sigma_{\mathrm{ser}}$ and $\tau_{\mathrm{ser}}$ model the shape feature of SER. The results of $\tau_{\mathrm{ser}}$ and $\sigma_{\mathrm{ser}}$ of 9 MCP-PMTs are $7.2\pm1.0$ and $1.62\pm0.06$.
...
\begin{figure}[!htbp]
    ...
    \end{subfigure}
    \caption{(a) Double-pulse example. The first pulse falls in the candidate window defined in sec.~\ref{sec:laserstage}. (b) The green and blue histograms are respectively the main and delayed pulse distributions. The filled histograms are waveforms containing the main and delayed pulses simultaneously.}
\end{figure}
...
The distribution of delay time from $t_r^{10}$ of main to after-pulse in Fig.~\ref{fig:afterpulse1d} indicates 5 characteristic peaks and structure at around \SI{300}{ns}, \SI{400}{ns}, \SI{600}{ns}, \SI{1200}{ns} and \SI{1700}{ns}, ratio being about $1:\sqrt{2}:\sqrt{4}:\sqrt{16}:\sqrt{32}$. These groups may originate from \ce{H^+}, \ce{H_2^+}, \ce{He^{+}}, \ce{CH_4^+}, and \ce{N_2^+} or \ce{O_2^+}. XENON1T~\cite{XENON1TTesting} and XMASS~\cite{Abe_2020} gave similar assumptions for the first peak. Similar works by JUNO~\cite{Zhao:2022gks} and KM3NeT~\cite{KM3NetTesting} claimed the first peak to be \ce{H_2^{+}}.

We use 5 gaussians $\sum_{i=1}^{5}{A_if_i^{\mathrm{AP}}(t;t_i,\sigma_i^2)}$ to model the 5 groups after substracting DCR, in which $A_i$, $t_i$, and $\sigma_i$ are the amplitudes, times, width of each after-pulse group (Table.~\ref{tab:afterpulse}). The amplitudes of the 5 groups vary greatly for different MCP-PMTs in Fig.~\ref{fig:afterpulsePeak}. Besides, we suggest the unexplained slow undulation in time interval $[\SI{2000}{ns},\SI{7000}{ns}]$ contributing about half of after-pulses is modeled with a uniform distribution.
\begin{table}.
    \centering
...
The mean charge in each time bin shown as the black line in Fig.~\ref{fig:afterpulse2d} indicates that 5 after-pulse groups contain large charge signals. The charge distributions in $[t_i-3\sigma_i,t_i+3\sigma_i]$ scaled to the same dark noise count for each after-pulse group except 2nd group due to overlap with 1st group is shown in Fig.~\ref{fig:afterpulsecharge} and we suggest to use the same charge parameters with 1st group. The low charge area is consistent with dark noise and the large charge events contributed from afterpulse are fitted with a gaussian $f^{AP_Q}_i(Q;Q_i,\sigma_{Q_i}^2)$ (Table.~\ref{tab:afterpulse}). The mean charge of each group without the 2nd group of 6 high statistics PMTs in Fig.~\ref{fig:afterpulseQ} shows a decreased relationship between charge and time. The charge distribution of 3rd low statistics group cannot be fitted well and therefore $\sigma_{Q_3}$s are large. 
