...
Over all the waveform samples, a Gaussian $f(t;t_0,\sigma_{t0}^2)$ is fitted to the distribution of $t_p-t_{\mathrm{trig}}$ of pulses whose $V_p$ exceeds \SI{5}{ADC}. We define a new candidate window, about \SI{30}{ns} long, as $[t_0-5\sigma_{t0}, t_0+5\sigma_{t0}]$ to calculate new $t_p$ and $V_p$ by repeating the above procedures.  With such a candidate window the dark counts reduced by 10, we shall conduct the charge and time characterization of the single PE pulse (main pulse) in the following sections.
...
To describe the peak shape of the $Q$ distribution, a Gaussian function $N^{\mathrm{1e}}f(Q;Q_0,\sigma^2_{Q_0})$ is fitted to the interval $[0.65Q_0, 1.35Q_0]$ via the modified least-square (MLS)~\cite{Cowan1998StatisticalDA} as the red line in Fig.~\ref{fig:triggercharge}. The mean $\overline{Q}$ and sample variance $s^2_{Q}$ of $Q$ of $N^{\mathrm{hit}}$ selected main pulses, with $V_p>\SI{3}{ADC}$ and $Q>0.25Q_0$, remove the influence of the pedestal and describe the long tail~\cite{JUNOLongtail}.
% The $V_p$ distribution with \SI{1}{ADC} bin width in Fig.~\ref{fig:triggerpeak} 
The two cuts are complementary to exclude some noises with small $V_p$ but large $Q$~(Fig.~\ref{fig:triggercharge}). Besides, the Gaussian component ratio $N^{\mathrm{1e}}/N^{\mathrm{hit}}$ ($0.59\pm0.02$ for the nine MCP-PMTs) of charge distribution reflects the significance of long tail.
...
, in which $\mu_{\mathrm{SER}}$ is the time offset of the pulse, $\sigma_{\mathrm{SER}}$ and $\tau_{\mathrm{SER}}$ model the shape of SER. They are measured to be $\tau_{\mathrm{SER}} = \SI{7.2\pm1.1}{ns}$ and $\sigma_{\mathrm{SER}} = \SI{1.63\pm0.06}{ns}$ for the 9 MCP-PMTs.
...
The probabilities of pre-pulses $P_{\mathrm{pre}}$ and after-pulses $P_{\mathrm{after}}$ are calculated in the time interval [\SI{-100}{ns},\SI{-10}{ns}] ($T_{\mathrm{pre}}=90$\,ns) and [\SI{200}{ns},\SI{9800}{ns}] ($T_{\mathrm{after}}=9600$\,ns) relative to the main pulses respectively as the following equations

\begin{align}
    P_{\mathrm{pre}} = \frac{N^{\mathrm{pre}}}{N^\mathrm{hit}} - \mathrm{DCR}\cdot T_{\mathrm{pre}}\\
    P_{\mathrm{after}} = \frac{N^{\mathrm{after}}}{N^\mathrm{hit}} - \mathrm{DCR}\cdot T_{\mathrm{after}}
\end{align}
in which $N^{\mathrm{pre}}$ and $N^{\mathrm{after}}$ are the number of pre-pulses and after-pulses.  The small $P_{\mathrm{pre}}$ ($3\times10^{-5}\pm1.2\times10^{-4}$) is dominated by DCR at our low-occupancy setup, and $P_{\mathrm{after}}$ of 9 MCP-PMTs is $0.009\pm0.006$.
...
\begin{table}
    ...
    \begin{threeparttable}
        \begin{tabular}{c|c|c|c|c|c}
            ...
            $A_i/N^{\mathrm{hit}}/10^{-3}$&1.2$\pm$0.7&0.3$\pm$0.2&0.3$\pm$0.3&0.8$\pm$0.6&1.5$\pm$0.6\\
...
A regression method is developed to combine the light-source calibration and PDE measurements simultaneously. Note $I_n$ to be the light intensity of the $n$th run, $\alpha_j$ to be the light allocation ratio of the $j$th splitter channel (out of four and assumed to be stable across runs), $\epsilon_k$ to be the PDE of the $k$th PMT (out of one reference dynode and nine MCP PMTs). The PE counts in each waveform obey Poisson distribution $\pi(I_n\alpha_j\epsilon_k)$.

For convenience, the index of the reference PMT is set to 0. Note $\alpha_j^0\equiv\alpha_j/{\alpha_0}$, $\epsilon_k^0\equiv\epsilon_k/{\epsilon_0}$, $I_n^0\equiv I_n\alpha_0\epsilon_0$.  The hit rate $R_{njk}$ of the $k$th PMT at the $j$th channel in the $n$th run is
