...
\label{sec:dcr}
The dark noise mainly comes from the spontaneous thermionic electrons emitted from the photocathode~\cite{KM3NetTesting} and mimick PEs. The dark count rate (DCR) is $\frac{N_{\mathrm{noise}}}{N_{\mathrm{hit}}T_{\mathrm{DCR}}}$, in which $N_{\mathrm{noise}}$ is the noise count in the interval of $[\SI{-200}{ns},\SI{-150}{ns}]$ relative to main pulse with $T_{\mathrm{DCR}}=\SI{50}{ns}$ and $N_{\mathrm{hit}}$ is the number of waveforms with main pulses.
